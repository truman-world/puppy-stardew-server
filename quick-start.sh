...
show_next_steps() {
    ...
    echo "     ${CYAN}docker attach puppy-stardew${NC}"
    echo "   - Enter your Steam Guard code from email/mobile app"
    echo "   - Press ${YELLOW}Ctrl+P Ctrl+Q${NC} to detach (NOT Ctrl+C!)"
    echo ""

    echo -e "${BOLD}3. Initial setup via VNC (first time only):${NC}"
    echo "   - Download a VNC client (RealVNC, TightVNC, etc.)"
    echo "   - Connect to: ${CYAN}$(get_server_ip):5900${NC}"
    echo "   - Password: ${CYAN}$(grep VNC_PASSWORD .env | cut -d'=' -f2)${NC}"
    ...
    echo "   - Server should appear, or manually enter: ${CYAN}$(get_server_ip):24642${NC}"
    echo ""

    echo -e "${BOLD}Useful commands:${NC}"
    echo "   View logs:        ${CYAN}docker logs -f puppy-stardew${NC}"
    echo "   Restart server:   ${CYAN}docker compose restart${NC}"
    echo "   Stop server:      ${CYAN}docker compose down${NC}"
    echo "   Check health:     ${CYAN}./health-check.sh${NC}"
    echo "   Backup saves:     ${CYAN}./backup.sh${NC}"
    echo ""
